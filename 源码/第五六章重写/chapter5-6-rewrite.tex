...
\newcommand{\U}[1]{(\Z/#1\Z)^\times}
\newcommand{\Cyc}[1]{\Z/#1\Z}
\newcommand{\e}[1]{e\!\left(#1\right)}
\newcommand{\expi}[1]{\exp(2\pi i #1)}
\providecommand{\cond}{\operatorname{cond}}
\providecommand{\thmheiti}{}
...
把局部同构写开，可得到完全显式的坐标表示：

\begin{itemize}[leftmargin=2em]
  \item 若 $\alpha_0=0$，没有二次因子坐标。
  \item 若 $\alpha_0=1$，$\U2$ 为平凡群，也没有有效坐标。
  \item 若 $\alpha_0=2$，$\U4=\{\pm1\}$，用一个坐标 $u\in\Cyc2$ 表示 $(-1)^u$。
  \item 若 $\alpha_0\ge3$，用两个坐标
  \(
    u\in\Cyc2,\qquad v\in\Cyc{2^{\alpha_0-2}},
  \)
  表示
  \(
    (-1)^u5^v\pmod {2^{\alpha_0}}.
  \)
  \item 对每个奇素数幂 $p_i^{\alpha_i}$，用坐标
  \(
    r_i\in \Cyc{\varphi(p_i^{\alpha_i})}
  \)
  表示
  \(
    g_i^{r_i}\pmod {p_i^{\alpha_i}}.
  \)
\end{itemize}

因此在这些坐标下，群运算就是各坐标相加。这是后面“指标组”的来源。
...
\(
  C(m;p^\alpha)
  =
  \sum_{\substack{a\bmod p^\alpha\\p\nmid a}}
